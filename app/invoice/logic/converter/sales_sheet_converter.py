...
class SalesSheetConverter(BaseHTMLDataToExcelConverter):
    """
    売上表テーブルのhtmlを請求書明細書エクセルに整形するクラス
    """
    ...
    def convert_to_excel(self, period:str):
        ...
        df = DataFrameUtils(
            self.data_source,
            columns=["店舗番号", '店舗名', "現金売上", "クレジット", "金券・他", "商業施設掛", "ショップ掛", "売上金額合計", "通帳入金合計(調整金含む）", "通帳入金調整金"]
        )
        df.save_to_excel(f"{self.output_path}/売上表_{period}.xlsx")
